...
export default function Transactions() {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [transactions, filter, goalFilter, searchQuery, customCategories, settings.language]);
  ...
  const remainingCount = filtered.length - paginatedFiltered.length;
            ...
            {historyOpen && (
              <>
                ...
                <div className="px-4 pt-2 pb-2 space-y-2 border-t border-[#F2F4F7]">
                  <div className="flex bg-[#F2F4F7] rounded-lg p-0.5" role="tablist" aria-label={t('tx_title')}>
                    {FILTER_TABS.map(tab => (
                      <button
                        key={tab.key}
                        role="tab"
