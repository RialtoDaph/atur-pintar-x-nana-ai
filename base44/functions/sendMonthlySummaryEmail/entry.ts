...
Deno.serve(async (req) => {
  try {
    ...
    const content = `
      ...
      <p style="margin:0 0 28px;font-size:14px;color:#64748B;text-align:center;">Hei <strong style="color:#1A1A1A;">${userName}</strong>! Ini ringkasan keuanganmu bulan ini.</p>
    ...
    await resend.emails.send({
      ...
      to: user.email,
      subject: `📊 Ringkasan Keuangan ${monthName} Kamu — Atur Pintar`,
      html: emailBody,
    });
